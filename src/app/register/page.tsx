...
export default function RegisterPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setErr(prev => ({ ...prev, server: "" })); // Hilangkan error server saat ngetik ulang
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Sabuk pengaman agar tidak crash kalau server error
        const text = await res.text();
        try {
          const data = JSON.parse(text);
          setErr(prev => ({ ...prev, server: data.message || "Gagal mendaftar." }));
        } catch (e) {
          setErr(prev => ({ ...prev, server: "Terjadi kesalahan fatal di server." }));
        }
      }
    } catch (error) {
      setErr(prev => ({ ...prev, server: "Gagal terhubung ke server. Cek koneksi Anda." }));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-center text-gray-500 text-sm mb-6">Lengkapi data diri Anda di bawah ini</p>
